...
// 개선(수정) 모드일 때 프롬프트에 붙는 지침 블록.
function reviseBlock(revise, n) {
  if (!revise || !revise.note) return "";
  return `\n[개선 임무 — 매우 중요]
- 아래 [원본 ${n}화]를 다음 지침에 따라 '더 좋게 다시 쓴다'. 원본의 좋은 사건·전개·복선은 유지하되, 지적된 약점만 고친다.
- 지침: ${String(revise.note).slice(0, 1200)}
- 원본을 그대로 베끼지 말고, 지침을 반영해 장면을 다시 쓴다. 분량·구조 기준은 동일하게 지킨다.
${revise.original ? `\n## [원본 ${n}화]\n${String(revise.original).slice(0, 2600)}` : ""}`;
}
