...
function buildChapterFirstPrompt({ input, n, prevText, ctx = {}, total = 25 }) {
  const system = `너는 웹소설 본문 작가다. 확정된 기획/세계관/시즌 설계와 직전 화에 이어 ${n}화를 쓴다. 지금은 이 회차의 [전반부]만 쓴다.

[시즌 위치] ${pacingHint(n, total)}
...
function buildChapterSecondPrompt({ input, n, prevText, ctx = {}, firstText, total = 25, isFinale = false }) {
...
- 마지막은 줄을 바꿔 '## 시즌 피날레'와, 다음 시즌으로 이어질 여운(차기 떡밥) 한 줄로 마무리한다.`
  ...
  const system = `너는 같은 ${n}화를 마무리하는 작가다. 아래 [이번 화 전반부]에 자연스럽게 이어 [후반부]를 써서 회차를 완성한다. (총 ${total}화 시즌의 ${n}화)
