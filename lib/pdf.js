...
// 압축 폭탄(zip bomb) 방어: 단일 스트림 inflate 출력 상한과 전체 누적 상한.
const MAX_INFLATE_BYTES = 12_000_000; // 스트림당 inflate 출력 상한(~12MB)
const MAX_TOTAL_TEXT = 4_000_000;     // 추출 텍스트 누적 상한(~4MB) — 이후 호출부에서 20k로 다시 컷
...
function extractTextFromPdf(buf) {
  ...
  while ((m = re.exec(latin)) !== null) {
    ...
    let content = null;
    try {
      content = zlib.inflateSync(raw, { maxOutputLength: MAX_INFLATE_BYTES }).toString("latin1");
    } catch {
      try { content = zlib.inflateRawSync(raw, { maxOutputLength: MAX_INFLATE_BYTES }).toString("latin1"); } catch { content = null; }
    }
    if (content === null) {
    ...
    if (content && /\bT[Jj]\b/.test(content)) {
      const piece = extractStringsFromContent(content);
      parts.push(piece);
      acc += piece.length;
      if (acc > MAX_TOTAL_TEXT) break; // 누적 상한 초과 시 중단
    }
    re.lastIndex = end + 9;
  }
  let text = parts.join("\n");
