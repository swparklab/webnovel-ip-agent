...
function foresightMd(input, p) {
  ...
  const pb = getPlaybook(input.genre, input.subgenre);
  return [
    `## 제작 판단`,
...
/* 작품개요 5요소 — 세계관 바이블/시즌 설계 앞에 붙는 자극적 피치(결정론적 폴백). */
function overviewMd(input, p, opts = {}) {
  const isSf = p.family === "sf" || Boolean(sfProfiles[input.genre]);
  const matLabel = isSf ? "과학소재" : "핵심 소재";
  const name = personName(input.protagonist);
  const ai = input.aiEntity || (isSf ? "핵심 시스템" : "핵심 인물");
  const foe = input.antagonist || "거대한 압력";
  const want = input.desire || "빼앗긴 것";
  const mat = input.coreTech || (p.tags || []).join(", ");
  const firstMat = (mat.split(",")[0] || mat).trim();
  const logline = input.logline ||
    `${name}이(가) ${foe} 앞에서 ${want}을(를) 되찾기 위해 모두가 금지한 단 하나의 선택을 한다.`;
  const premise = firstClause(input.sfPremise, p.promise);
  const story = [
    `${firstClause(input.socialShift, p.promise)} 그 질서 속에서 ${name}은(는) 가장 낮은 자리에서 시작한다.`,
    `${ai}이(가) ${name}의 운명을 뒤흔드는 사건이 터지고, ${name}은(는) ${firstMat}의 비밀에 손을 댄다. ${p.firstCliff || "그 순간, 돌이킬 수 없는 일이 시작된다."}`,
    opts.season
      ? `시즌이 진행될수록 ${foe}의 진짜 얼굴이 드러나고, ${name}은(는) ${input.seasonGoal || "판을 뒤집을 단 하나의 증거"}에 다가선다. 그러나 그 진실은 결국 ${name} 자신을 겨눈다.`
      : `${foe}의 진짜 목적이 드러나는 순간, ${name}은(는) 자신을 구할지 모두를 구할지 사이에서 찢긴다.`,
  ].join("\n\n");
  return [
    `## 작품개요`,
    `- **작품장르**: ${p.label} · ${isSf ? "근미래 SF 스릴러(혼합형)" : "장르 혼합형"}`,
    `- **${matLabel}**: ${mat}`,
    `- **로그라인**: ${logline}`,
    `- **기획의도**: 목표 독자는 ${input.targetReader || `${p.readerReward}을(를) 즐기는 독자`}. 「${premise}」라는 질문을 ${p.readerReward}(으)로 체감시킨다.`,
    `- **줄거리**:`,
    ``,
    story,
  ].join("\n");
...
function buildLocalReport(input) {
  ...
  const overview = overviewMd(input, p);
  const overviewSeason = overviewMd(input, p, { season: true });
  ...
  return {
    ...
    agents: {
      ...
      world: mk("world", "세계관 바이블", ["world", "timeline"], `${overview}\n\n${isSf ? worldMd(input, p) : genericWorldMd(input, pb)}`),
      plot: mk("plot", "Plot Engine", ["season"], `${overviewSeason}\n\n${isSf ? plotMd(input, p) : genericPlotMd(input, pb)}`),
