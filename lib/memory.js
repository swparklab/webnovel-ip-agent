...

{
  ...
  "canon": ["앞으로 바뀌면 안 되는 확정 사실(능력 한계·관계·사망·소유·비밀 공개 등). 없으면 빈 배열"]
}`;
...
function composeStorySoFar(memories, opts = {}) {
  ...
  // 확정 설정(canon): 중복 제거 후 누적.
  const canonSeen = new Set();
  const canon = [];
  nums.forEach((k) => {
    (memories[k].canon || []).forEach((c) => {
      const key = norm(c);
      if (key && !canonSeen.has(key)) { canonSeen.add(key); canon.push(c); }
    });
  });
...
module.exports = { buildSynopsisPrompt, parseMemory, localMemory, composeStorySoFar };
