...
function streamMessageCli(opts) {
  ...
  return new Promise((resolve, reject) => {
    ...
    let stdoutBuf = "";
    let settled = false;
    let resultError = null; // result 이벤트가 is_error로 표시한 오류 메시지(있으면 close에서 reject)
    ...
    child.stdout.on("data", (chunk) => {
      ...
      while ((nl = stdoutBuf.indexOf("\n")) !== -1) {
        ...
        if (evt.type === "result") {
          ...
          // CLI가 오류를 result로 전달한 경우(종료코드가 0이어도) 본문으로 착각하지 않게 표시.
          if (evt.is_error || evt.subtype === "error") {
            resultError = (typeof evt.result === "string" && evt.result.trim())
              ? evt.result.trim()
              : "Claude Code가 오류 결과를 반환했습니다.";
          }
        }
      }
    });
    ...
    child.on("close", (code) => {
      ...
      // is_error로 표시된 오류 결과를 원고/산출물로 흘려보내지 않는다.
      if (resultError) {
        const hint = /not.*log|login|authenticat|credit|subscription|usage|limit/i.test(resultError)
          ? " (구독 로그인/사용량을 확인하세요)"
          : "";
        reject(new ClaudeCliError(`Claude Code 오류: ${resultError.slice(0, 400)}${hint}`));
        return;
      }
