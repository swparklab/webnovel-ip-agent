...
const { resolveMedium, mediumLabel, formatLabel, buildMediumBlock } = require("./medium");

/** 매체가 웹소설이 아니면, 그 매체의 성공방정식·연출·구조를 기획에 주입할 블록을 만든다. */
function mediaFraming(medium, format) {
  const m = resolveMedium(medium);
  if (m === "webnovel") return "";
  return `\n[매체 맥락 — 이 기획은 '${mediumLabel(m)}'(${formatLabel(format)})용이다. 회차 연재가 아니라 이 매체의 문법으로 채운다]\n${buildMediumBlock(m, format)}\n- 위 매체 성공 방정식·연출에 맞게 각 필드를 채운다. (seasonGoal은 '작품 목표', protagonist는 '핵심 인물/화자' 등 매체 맥락으로 해석)`;
}
...
function buildIdeatePrompt(idea, genre, subgenre, blendGenres, medium, format) {
  ...
  const mediaBlock = mediaFraming(medium, format);
...
/**
 * '빈 칸만 보강' 모드. 작가가 이미 채운 필드는 '확정'으로 고정하고,
 * 비어 있는 필드만 채워진 항목·장르 흥행 문법과 모순 없이 채운다.
 */
function buildCompletePrompt(input, genre, subgenre, blendGenres, medium, format) {
  const g = getPlaybook(genre, subgenre);
  const mediaBlock = mediaFraming(medium, format);
  const filled = IDEATE_FIELDS.filter(([k]) => String(input[k] ?? "").trim());
  const empty = IDEATE_FIELDS.filter(([k]) => !String(input[k] ?? "").trim());
  const idea = String(input.logline || input.sfPremise || input.ipTitle || "").trim();

  const filledBlock = filled.length
    ? filled.map(([k, desc]) => `- ${k} (${desc}): ${String(input[k]).trim()}`).join("\n")
    : "(채워진 항목 없음 — 아래 빈 항목을 장르 흥행 문법에 근거해 새로 설계)";
  const emptySpec = empty.map(([k, desc]) => `  "${k}": "${desc}"`).join(",\n");

  const system = `너는 웹소설 기획 보조다. 작가가 일부만 채운 Core IP 폼을 받아, '비어 있는 항목만' 채워 완성한다.

[가장 중요한 규칙]
- 작가가 이미 채운 항목은 '확정'이다. 절대 바꾸거나 무시하지 마라.
- 비어 있는 항목만 채우되, 채워진 항목들과 모순 없이 자연스럽게 이어지도록 설계한다(인물 이름·세계 규칙·톤·갈등이 서로 맞물려야 한다).
- 장르 흥행 문법(결핍→특권→회차 검증→즉시 보상→세계 확장)에 근거해 비어 있는 항목을 매력적으로 채운다.
...
[출력 형식 — 매우 중요]
- 오직 JSON 객체 하나만 출력한다. 코드펜스·설명·인사말 금지.
- '비어 있던 항목'의 키만 포함한다(확정 항목은 출력하지 마라). 모든 값은 한국어 문자열.
${empty.length ? `\n{\n${emptySpec}\n}` : "{}"}`;

  const user = `장르: ${g.label} (${g.key})${idea ? `\n작품 단서: ${idea}` : ""}

위 '확정 항목'과 모순 없이, 비어 있는 항목만 채운 JSON을 작성하라.`;
...
/** '빈 칸만 보강' 폴백. 비어 있는 필드만 장르 프리셋으로 채운다(확정 필드는 건드리지 않음). */
function localComplete(input, genre, subgenre) {
  const g = getPlaybook(genre, subgenre);
  const preset = g.preset || {};
  const out = {};
  FIELD_KEYS.forEach((k) => {
    const cur = String(input[k] ?? "").trim();
    if (!cur && preset[k] != null) out[k] = preset[k];
  });
  // coreTags가 비었으면 장르 장치 태그로.
  if (!String(input.coreTags ?? "").trim() && !out.coreTags && Array.isArray(g.devices)) {
    out.coreTags = g.devices.map((d) => d[0]).slice(0, 6).join(", ");
  }
  return out;
}

module.exports = {
  buildIdeatePrompt, buildCompletePrompt, extractFields,
  localIdeate, localComplete, IDEATE_FIELDS, FIELD_KEYS,
};
