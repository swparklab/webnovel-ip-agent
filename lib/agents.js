...
function buildInputBlock(input) {
  ...
  const bibleBlock = bible.length
    ...
    : null;
  // AI미래학자 박성우(foresight) 모드: 실재 AI·기술 신호에 근거한 미래예측 렌즈를 강제.
  const foresightBlock = input.foresight
    ? `\n# AI FORESIGHT 렌즈 (AI미래학자 박성우 모드) — 최우선 구속
- 이 작품은 'AI 미래예측(Foresight)'을 핵심 동력으로 삼는다. 실재하는 AI·기술 신호(현재 연구·산업 동향)에서 출발해, 가까운 미래로의 합리적 외삽(extrapolation)으로 세계와 사건을 설계한다.
- 마법 같은 임의 설정이 아니라 '왜 그렇게 될 수밖에 없는가'의 인과(기술→사회→권력→개인)를 한 단계씩 보여준다. 과장된 디스토피아 클리셰 대신 구체적이고 그럴듯한 변화에 초점을 둔다.
- 미래 기술의 '능력'뿐 아니라 '대가·한계·오작동·악용'에서 갈등과 떡밥을 만든다.
- 단, 설정 나열로 흐르지 말 것 — 통찰은 장면·사건·인물의 선택으로 보여준다(웹소설 흥행 공식은 그대로 유지).`
    : null;
  return [
    // 🔒 감독 원시트 LOCK(있으면 최상단) — 모든 장르·산출물이 이 잠금값을 따른다.
    buildOneSheetLockBlock(input.oneSheet, input.medium) || null,
    // 🌍 세계 설정 LOCK(있으면) — 세계 규칙·금기·프롭을 회차 간 고정.
    buildWorldLockBlock(input.worldLock) || null,
    `# 작품 입력 데이터`,
    ...
    foresightBlock,
    buildAiFilmDoctrineBlock(input) || null,
    buildSteeringBlock(input.steering) || null,
    bibleBlock,
    input.references
