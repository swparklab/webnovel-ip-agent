...
const { buildPlaybookBlock, checklistBlock } = require("./playbook");
...
function buildInputBlock(input) {
  ...
  return [
    ...
    `- 연재 플랫폼: ${PLATFORM_LABELS[input.platform] || "카카오페이지"}`,
    ...
    ``,
    buildPlaybookBlock(input.genre),
  ]
...
[출력 구조]
## 1화. (회차 제목)
(1,800~2,600자 분량의 실제 원고 본문. 문단 단위로 작성. 대사·묘사·내면을 섞는다.
