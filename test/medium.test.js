...
test("㉚ 매체별 전문 도구팩: 6 매체 모두 도구 보유 + 생성기/분석기 프롬프트·폴백 무결", () => {
  const tk = require("../lib/media-toolkit");
  const meds = ["documentary", "film", "animation", "drama", "advertising", "webnovel"];
  for (const m of meds) {
    const list = tk.toolkitList(m);
    assert.ok(list.length >= 5, `${m}: 전문 도구 5개 미만`);
    const meta = tk.toolkitMeta(m);
    assert.ok(meta.tools.every((t) => t.id && t.label && t.kind && t.group), `${m}: 메타 필드 누락`);
    for (const t of list) {
      if (t.kind === "analyzer") {
        const p = tk.buildToolkitAnalyzePrompt({ medium: m, tool: t.id, input: { ipTitle: "X" }, text: "표 | 감정 색 80" });
        assert.ok(p && p.system.includes("JSON"), `${m}/${t.id}: 분석기 프롬프트 불량`);
        const l = tk.localToolkitAnalyze({ medium: m, tool: t.id, text: "표 | 감정 80" });
        assert.ok(l.overall > 0 && Array.isArray(l.flags) && l.fallback, `${m}/${t.id}: 분석기 폴백 불량`);
      } else {
        const p = tk.buildToolkitPrompt({ medium: m, tool: t.id, input: { ipTitle: "X" } });
        assert.ok(p && p.system.includes(t.label), `${m}/${t.id}: 생성기 프롬프트 불량`);
        const l = tk.localToolkit({ medium: m, tool: t.id, input: { ipTitle: "X" } });
        assert.ok(l.includes(t.label) && l.length > 40, `${m}/${t.id}: 생성기 폴백 불량`);
      }
    }
  }
  // 분석기는 /api/toolkit으로 막혀야(전용 분석 엔드포인트 사용).
  const ana = tk.toolkitList("documentary").find((t) => t.kind === "analyzer");
  if (ana) assert.equal(tk.buildToolkitPrompt({ medium: "documentary", tool: ana.id }), null, "분석기가 생성기 경로로 빌드됨");
  // 다음작업 추천: 안 한 것 우선, 최대 3.
  const ns = tk.nextStepGuide("drama", []);
  assert.ok(ns.length >= 1 && ns.length <= 3, "다음작업 추천 개수 이상");
  // 파서: JSON 추출 + flags 정규화.
  const parsed = tk.parseToolkitAnalyze('{"overall":77,"scores":{"균형":60},"flags":["x"],"fixes":["y"],"verdict":"z"}');
